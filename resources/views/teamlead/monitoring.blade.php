                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 hover:shadow-xl transition-shadow duration-300">
                        <div class="flex justify-between items-center mb-6 pb-4 border-b-2 border-gray-100">
                            ...
                            @php
                                $projectPriorityTasks = $priorityTasksByProject[$project->id] ?? collect();
                                $highCount = $projectPriorityTasks->where('priority', 'high')->count();
                                $mediumCount = $projectPriorityTasks->where('priority', 'medium')->count();
                            @endphp
                        ...
                        <div class="space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar">
                            @forelse($projectPriorityTasks as $task)
                            <div class="p-4 bg-gradient-to-r 
                                {{ $task->priority === 'high' ? 'from-red-50 to-rose-50 border-l-4 border-red-600' : 'from-amber-50 to-yellow-50 border-l-4 border-amber-600' }} 
                                rounded-xl hover:shadow-md transition-shadow">
                                <div class="flex justify-between items-start mb-3">
                                    <h4 class="text-sm font-bold text-gray-900 flex-1 pr-2">{{ $task->title }}</h4>
                                    <span class="px-2.5 py-1 rounded-lg text-xs font-bold uppercase flex-shrink-0 shadow-sm
                                        {{ $task->priority === 'high' ? 'bg-red-600 text-white' : 'bg-amber-600 text-white' }}">
                                    ...
                                    <div class="flex items-center gap-1.5 text-xs text-gray-600">
                                        @if($task->assignedUser)
                                            ...
                                            <img src="{{ asset('storage/' . $task->assignedUser->avatar) }}" 
                                                    alt="{{ $task->assignedUser->full_name }}"
                                            ...
                                            <div class="w-6 h-6 bg-gradient-to-br from-primary-600 to-primary-800 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-sm">
                                                {{ strtoupper(substr($task->assignedUser->full_name ?: $task->assignedUser->username, 0, 2)) }}
                                            </div>
                                            @endif
                                            <span class="font-medium">{{ $task->assignedUser->full_name ?: $task->assignedUser->username }}</span>
                                        @else
                                            <div class="w-6 h-6 bg-gray-400 rounded-full flex items-center justify-center text-white text-xs">
                                                <i class="fas fa-user-slash"></i>
                                            </div>
                                            <span class="font-medium text-gray-500">Unassigned</span>
                                        @endif
                                    </div>
                                    @if($task->due_date)
                                    <span class="bg-white px-2.5 py-1 rounded-lg text-xs font-bold text-gray-700 shadow-sm border border-gray-200">
                                        <i class="fas fa-clock {{ \Carbon\Carbon::parse($task->due_date)->isPast() ? 'text-red-500' : 'text-amber-500' }}"></i> 
                                ...
                                @php
                                    ...
                                    $taskSubtasksTotal = $taskSubtasks->count();
                                    $taskSubtasksDone = $taskSubtasks->where('status', 'done')->count();
                                    $taskProgress = $taskSubtasksTotal > 0 ? round(($taskSubtasksDone / $taskSubtasksTotal) * 100) : 0;
                                @endphp
                                
                                @if($taskSubtasksTotal > 0)
                                <div class="h-2 bg-white rounded-full overflow-hidden shadow-inner mt-2">
                                    <div class="h-full bg-gradient-to-r {{ $task->priority === 'high' ? 'from-red-500 to-red-700' : 'from-amber-500 to-amber-700' }} rounded-full" 
                                            style="width: {{ $taskProgress }}%"></div>
                                </div>
                                <p class="text-xs text-gray-600 mt-1">{{ $taskSubtasksDone }}/{{ $taskSubtasksTotal }} subtasks completed</p>
                                @endif
                            </div>
                            @empty
                            <div class="text-center py-8">
                                ...
                                <p class="text-sm text-gray-500">No priority tasks</p>
                            </div>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 hover:shadow-xl transition-shadow duration-300">
                        <div class="flex justify-between items-center mb-6 pb-4 border-b-2 border-gray-100">
                            ...
                            @php
                                $projectActivities = $recentActivitiesByProject[$project->id] ?? collect();
                            @endphp
                            <span class="bg-primary-100 text-primary-700 px-3 py-1 rounded-full text-xs font-bold">
                                {{ $projectActivities->count() }} Updates
                            </span>
                        </div>
                        ...
                        <div class="relative max-h-[500px] overflow-y-auto custom-scrollbar">
                            ...
                            <div class="space-y-1">
                                ...
                                @php
                                    ...
                                    if (stripos($activity->comment_text, '[APPROVED]') !== false) {
                                        $status = 'approved';
                                        $statusColor = 'blue';
                                        $statusIcon = 'fa-check-circle';
                                    } elseif (stripos($activity->comment_text, '[REJECTED]') !== false) {
                                        $status = 'rejected';
                                        $statusColor = 'red';
                                        $statusIcon = 'fa-times-circle';
                                    } elseif (stripos($activity->comment_text, '[BLOCKED]') !== false || stripos($activity->comment_text, '[BLOCKER]') !== false) {
                                        $status = 'blocked';
                                        $statusColor = 'orange';
                                        $statusIcon = 'fa-exclamation-triangle';
                                    } elseif (stripos($activity->comment_text, '[REVIEW]') !== false) {
                                        $status = 'review';
                                        $statusColor = 'purple';
                                        $statusIcon = 'fa-eye';
                                    }
                                ...
                                <div class="relative flex gap-3 pb-4 hover:bg-gray-50 p-2 rounded-lg transition-colors">
                                    <div class="relative z-10 flex-shrink-0">
                                        ...
                                        <img src="{{ asset('storage/' . $activity->user->avatar) }}" 
                                                ...
                                                class="w-9 h-9 rounded-full object-cover border-3 border-white shadow-md">
                                        @else
                                        <div class="w-9 h-9 bg-gradient-to-br from-{{ $statusColor }}-600 to-{{ $statusColor }}-800 rounded-full flex items-center justify-center text-white font-bold text-sm border-3 border-white shadow-md">
                                            {{ strtoupper(substr($activity->user->full_name ?: $activity->user->username, 0, 2)) }}
                                        </div>
                                        @endif
                                    </div>
                                    <div class="flex-1 pt-1">
                                        <p class="text-xs text-gray-900 mb-1">
                                            <strong class="font-bold">{{ $activity->user->full_name ?: $activity->user->username }}</strong> 
                                            <span class="text-{{ $statusColor }}-600 font-semibold">{{ strtoupper($status) }}</span> 
                                            <span class="text-{{ $statusColor }}-600 font-semibold">"{{ $activity->card->title }}"</span>
                                        </p>
                                    ...
                                    <div class="flex-shrink-0">
                                        <span class="inline-flex items-center justify-center w-6 h-6 bg-{{ $statusColor }}-100 rounded-full">
                                            <i class="fas {{ $statusIcon }} text-{{ $statusColor }}-600 text-xs"></i>
                                        </span>
                                    </div>
                                </div>
                                @empty
                                <div class="text-center py-8">
                                    <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-3">
                                        <i class="fas fa-rss text-gray-400 text-2xl"></i>
                                    </div>
